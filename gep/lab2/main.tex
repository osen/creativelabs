...
Smart pointers and RAII provides \Name{C++} with fantastic (and quite unique) functionality when dealing with memory, however it is a little bit long winded to type.
We can address this by using \Name{typedefs} or \Name{MACROS}. The simplest way would be to define in your engine's headers the following:
